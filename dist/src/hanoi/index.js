...
app.post('/', validateDisks(), async (c) => {
    ...
    const { disks, origin = originDefaultName, destiny = destinyDefaultName, helper = helperDefaultName, } = payload ?? {};
    ...
    customLogger('Hanoi solution: ', result.totalMoves.toString(), result.time.toString());
    return c.json({
        data: result,
        ok: true,
        error: null,
